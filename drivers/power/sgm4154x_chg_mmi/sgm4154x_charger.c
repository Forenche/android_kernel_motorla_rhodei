...
#define sgm_info(fmt, ...)	((void)0)
#define sgm_dbg(fmt, ...)	((void)0)
...
/* enable dynamic adjust battery voltage */
static int sgm4154x_set_chrg_adjust_volt(struct sgm4154x_device *sgm, int chrg_volt, int adjust)
{
	int ret;
	int reg_val;
	...
	reg_val = (chrg_volt-SGM4154x_VREG_V_MIN_uV) / SGM4154x_VREG_V_STEP_uV + adjust;
	reg_val = reg_val<<3;
...
static int sgm4154x_detected_qc20_hvdcp(struct sgm4154x_device *sgm, int *charger_type)
{
	...
	sgm_info(sgm->dev, "vbus voltage now = %d\n", vbus_voltage);

	if (vbus_voltage > MMI_HVDCP2_VOLTAGE_STANDARD) {
		sgm_info(sgm->dev, "QC20 charger detected\n");
		*charger_type = POWER_SUPPLY_TYPE_USB_HVDCP;
	} else {
		sgm_info(sgm->dev, "charger type is not HVDCP\n");
	}
	...
	sgm4154x_get_usb_voltage_now(sgm, &vbus_voltage);
	sgm_info(sgm->dev, "vbus voltage now = %d after qc20 detected\n", vbus_voltage);
...
static int sgm4154x_set_dynamic_vindpm_track(struct sgm4154x_device *sgm, int track_uv)
{
	...
	sgm_info("%s track_uv: %d, reg_val: %d\n", __func__, track_uv, reg_val);
...
static int sgm4154x_set_icl(struct charger_device *chg_dev, u32 uA)
{
	...
	sgm_info("%s set icl curr = %d\n", __func__, uA);

	sgm->input_current_cache = uA;

	if (sgm->mmi_is_qc3_authen) {
		sgm_info(sgm->dev, "hvdcp detecting now\n");
		return 0;
	}
...
void sgm4154x_rerun_apsd(struct sgm4154x_device * sgm)
{
	...
	sgm_info(sgm->dev, "re-running APSD\n");
...
static bool sgm4154x_is_rerun_apsd_done(struct sgm4154x_device * sgm)
{
	...
	sgm_info("%s:rerun apsd %s", __func__, result ? "done" : "not complete");
	return result;
}
...
/* enable dynamic adjust battery voltage */
static void sgm4154x_adjust_constant_voltage(struct sgm4154x_device *sgm, int vbat)
{
	int ret, t1, t2;
	int ft[4] = {0, 0x40, 0xC0, 0x80};
	int reg = 0;

	if (sgm->final_cv > vbat && (sgm->final_cv - vbat) < 8000)
		return;
	ret = mmi_regmap_read(sgm, SGM4154x_CHRG_CTRL_a, &reg);
	if ((ret || (reg & 0x60)) && sgm->final_cv > vbat)
		return;
	if (sgm->final_cv > vbat) {
		sgm->cv_tune++;
	} else if (sgm->cv_tune > 0) {
		sgm->cv_tune--;
	}
	sgm->cv_tune = min(sgm->cv_tune, 12); //limit to max 12 - 96mV
	dev_err(sgm->dev, "cv_tune=%duV\n", sgm->cv_tune*8000);
	t1 = sgm->cv_tune / 4;
	t2 = sgm->cv_tune - t1*4;
	if (t2 == 2 || t2 == 3)
		t1++;
	ret = mmi_regmap_update_bits(sgm, SGM4154x_CHRG_CTRL_f, SGM4154x_VREG_FT_MASK, ft[t2]);
	ret |= sgm4154x_set_chrg_adjust_volt(sgm, sgm->final_cv, t1);
	if (!ret)
		return;
	dev_err(sgm->dev, "failed to tune cv, reset the tuning\n");
	sgm->cv_tune = 0;
...
static void charger_monitor_work_func(struct work_struct *work)
{
	...
	if(sgm->enable_dynamic_adjust_batvol){
		...
		if (sgm->battery) {
			power_supply_get_property(sgm->battery, POWER_SUPPLY_PROP_CURRENT_NOW, &val_battery);
			if(val_battery.intval < 10000 && state.chrg_stat != SGM4154x_TERM_CHRG) {
				pr_err("VBUS present but no charging current \n");
				//sgm4154x_dump_register(sgm);
			}

			ibat_ua = val_battery.intval;
			power_supply_get_property(sgm->battery, POWER_SUPPLY_PROP_VOLTAGE_NOW, &val_battery);
			vbat_uv = val_battery.intval;
			dev_err(sgm->dev, "vbat=%duV, cv=%duV\n", vbat_uv, sgm->final_cv);
...
static int sgm4154x_detected_qc30_hvdcp(struct sgm4154x_device *sgm, int *charger_type)
{
	...
	sgm4154x_get_usb_voltage_now(sgm, &vbus_voltage);
	sgm_info(sgm->dev, "%s vbus voltage now = %d after detected qc30\n", __func__,vbus_voltage);
#endif
...
static int sgm4154x_detected_qc3p_hvdcp(struct sgm4154x_device *sgm, int *charger_type)
{
	...
	for (i = 0;i < 3; i++) {
		ret = sgm4154x_qc30_step_up_vbus(sgm);
		if (ret)
			dev_err(sgm->dev, "%s qc3p step up vbus error\n", __func__);

		ret = sgm4154x_qc30_step_down_vbus(sgm);
		if (ret)
			dev_err(sgm->dev, "%s qc3p step down vbus error\n", __func__);
	...
	sgm_info(sgm->dev, "%s vbus voltage now = %d after detected qc3p\n", __func__,vbus_voltage);
	...
	if (sgm->mmi_qc3p_power != MMI_POWER_SUPPLY_QC3P_NONE) {
		*charger_type = POWER_SUPPLY_TYPE_USB_HVDCP_3P5;
		sgm_info(sgm->dev, "%s detected qc3p, qc3p power = %d\n",
		...
		enum mmi_qc3p_ext_iio_channels chan)
{
	...
	if (!chip->ext_iio_chans[chan]) {
		...
		if (IS_ERR(chip->ext_iio_chans[chan])) {
			...
			sgm_info(chip->dev, "Failed to get IIO channel %s, rc=%d\n",
...
int qc3p_start_detection(struct sgm4154x_device *chip)
{
	...
	sgm_info(chip->dev, "write SMB5_QC3P_START_DETECT IIO success\n");
	return 0;
}
...
bool qc3p_detection_done(struct sgm4154x_device *chip)
{
	...
	do {
		...
		sgm_info(chip->dev, "read SMB5_QC3P_DETECTION_READY IIO :%d\n",val);
...
int qc3p_read_charger_type(struct sgm4154x_device *chip)
{
	...
	sgm_info(chip->dev, "read SMB5_USB_REAL_TYPE IIO :%d\n",val);
	return val;
}
...
bool qc3p_update_policy(struct sgm4154x_device *chip )
{
	...
	sgm_info(chip->dev, "write SMB5_QC3P_START_POLICY IIO :%d\n",val);
	return val;
}
...
static int mmi_hvdcp_detect_kthread(void *param)
{
	...
	do {
		...
		down(&sgm->sem_dpdm);
		sgm_info(sgm->dev, "mmi_hvdcp_detect_kthread begin\n");
		...
		if (charger_type == POWER_SUPPLY_TYPE_USB_HVDCP_3 && !sgm->pd_active) {
			...
			switch(ret) {
				case WT_CHG_TYPE_QC2:
					charger_type = POWER_SUPPLY_TYPE_USB_HVDCP;
					sgm_info(sgm->dev, "HDVCP 2.0 detected\n");
					break;
				case WT_CHG_TYPE_QC3:
				case WT_CHG_TYPE_QC3P_18W:
					charger_type = POWER_SUPPLY_TYPE_USB_HVDCP_3;
					sgm_info(sgm->dev, "HDVCP 3.0 or qc3p 18W detected\n");
					break;
				case WT_CHG_TYPE_QC3P_27W:
					...
					break;
				default:
					sgm_info(sgm->dev, "No HDVCP detected\n");
					goto out;
			}
#endif
	...
	out:
		...
		up(&sgm->sem_dpdm);
		sgm_info(sgm->dev, "mmi_hvdcp_detect_kthread end\n");
	}while(!kthread_should_stop());

	sgm_dbg(sgm->dev, "qc3 kthread stop\n");
	return 0;
}
...
static void charger_detect_work_func(struct work_struct *work)
{
	...
	if (((!sgm->state.vbus_gd) || (sgm->state.chrg_type == SGM4154x_OTG_MODE))
		...
		sgm_info(sgm->dev, "%s:adapter removed\n", __func__);
		sgm->status &= ~SGM4154X_STATUS_PLUGIN;
		...
		sgm_info(sgm->dev, "%s:adapter plugged in\n", __func__);
		sgm->status |= SGM4154X_STATUS_PLUGIN;
...
static int sgm4154x_hw_init(struct sgm4154x_device *sgm)
{
	...
	/* enable dynamic adjust battery voltage */
	if(sgm->enable_dynamic_adjust_batvol)
		ret = sgm4154x_set_chrg_adjust_volt(sgm,
...
static int sgm4154x_vbus_regulator_register(struct sgm4154x_device *sgm)
{
	...
	if (IS_ERR(sgm->otg_rdev)) {
		...
		sgm_info("%s: register otg regulator failed (%d)\n", __func__, ret);
	}
	return ret;
}
...
static int sgm4154x_hw_chipid_detect(struct sgm4154x_device *sgm)
{
	...
	if (ret < 0)
	{
		sgm_info("[%s] read SGM4154x_CHRG_CTRL_b fail\n", __func__);
		return ret;
	}
...
static int sgm4154x_set_charging_current(struct charger_device *chg_dev, u32 uA)
{
	...
	sgm_info("%s set charging curr = %d, %s\n", __func__, uA, rc ? "failed" : "success");
...
static int sgm4154x_set_charging_voltage(struct charger_device *chg_dev, u32 uV)
{
	...
	/* enable dynamic adjust battery voltage */
	if(sgm->enable_dynamic_adjust_batvol)
	    rc = sgm4154x_set_chrg_adjust_volt(sgm, uV, 0);
	else
	    rc = sgm4154x_set_chrg_volt(sgm, uV);

	sgm_info("%s set charging volt = %d, %s\n", __func__, uV, rc ? "failed" : "success");
...
static int sgm4154x_set_boost_current_limit(struct charger_device *chg_dev, u32 uA)
{
	...
	sgm_info("%s set boost current limit = %d, %s\n", __func__, uA, rc ? "failed" : "success");

	return rc;
}

static int sgm4154x_get_qc3p_power(struct charger_device *chg_dev, int *qc3p_power)
{
	struct sgm4154x_device *sgm = dev_get_drvdata(&chg_dev->dev);

	*qc3p_power = sgm->mmi_qc3p_power;
...
static int sgm4154x_set_dp_dm(struct charger_device *chg_dev, int val)
{
	...
	case MMI_POWER_SUPPLY_DP_DM_DP_PULSE:
		...
		sgm_dbg(sgm->dev,"DP_DM_DP_PULSE rc=%d cnt=%d\n",
				rc, sgm->pulse_cnt);
		break;
	case MMI_POWER_SUPPLY_DP_DM_DM_PULSE:
		...
		if (!rc && sgm->pulse_cnt)
			sgm->pulse_cnt--;
		sgm_dbg(sgm->dev, "DP_DM_DM_PULSE rc=%d cnt=%d\n",
				rc, sgm->pulse_cnt);
		break;
			 ...
			 const struct i2c_device_id *id)
{
	...
	sgm_info(dev, "SGM4154x prob successfully.\n");
	return ret;
error_out:
...
static void sgm4154x_charger_shutdown(struct i2c_client *client)
{
    ...
    sgm_info("sgm4154x_charger_shutdown\n");
}
